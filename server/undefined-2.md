...
**작성 기준**: `collector/server.js`, `mqtt-handler.js`, `redis-storage.js`, `batch-scheduler.js`, `parse/*`, `influx/*`, `alarm/*`, `utils/*`
...
flowchart TB
    ...
    SRV --> WS["Socket.IO sensor-data-v2, latest-data-v2"]
```
...
요청 본문이 곧 `processBulkData` 입력이다. `validateBulkData`로 검증한다.
...
WebSocket v2는 bulk `data` 객체를 그대로 전송한다. 저장 시 채널 번호가 100 미만이면 `chN` → `ch(N+100)`으로 보정한다.
...
| `data.val01`\~`val04` | 각각 별도 Point (`field_name` 태그)  |
...
MQTT 경로는 `parseProxy.parseData`가 형식을 판별해 아래 구조로 통일한다. Redis 저장, 알람 판정, WebSocket 전송이 이 구조를 사용한다.
...
`storeSensorData`는 `zAdd`와 `zRemRangeByScore`를 `multi()` 파이프라인으로 함께 실행한다.
...
| --------- | ------------------------------------------- | -------------------------------------------------------------------- |
| MQTT 센서   | `{sensorId}_{channel}_{10m\|1h\|1d}`        | `sensor_10min_aggregated` / `hourly_aggregated` / `daily_aggregated` |
...
최신값 Map은 `updateLatestSensorDataV2` / `latestSensorDataV2`가 관리한다. 센서별 채널 Map을 유지하고 필드 단위로 `time` 문자열을 비교해 더 최신 값만 갱신한다. MQTT 일반 경로는 숫자 값을 `{ mean: number }`로 감싼 뒤 전송한다.
