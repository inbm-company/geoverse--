...
| 고주파 분석     | `fft`         | 8000  | **별도 호스트**에서 운영중 |

***

### Management (관리 서버)

관리 화면용 **메타데이터·인증·알람 설정** API다. 현장·노드·장비·위젯·임계값·사용자를 PostgreSQL에 CRUD하고, 알람 이력은 InfluxDB에서 조회한다. **측정값은 저장·수집하지 않는다.**
...
| 현장·계층 | `/api/projects`, `/api/nodes`, `/api/folders` | 현장, ltree 노드 트리, 폴더          |
| 장비    | `/api/devices`                                | 노드·폴더에 연결된 장비·채널 CRUD        |
| 카탈로그  | `/api/device-types`, `/api/sensor-types`      | 장비·센서 종류 마스터                 |
| 화면    | `/api/sensor-widgets`, `/api/cctv-cameras`    | 대시보드 위젯, CCTV                |
| 알람    | `/api/thresholds`, `/api/alarm-logs`          | 임계값 CRUD, 발송 이력 조회           |
| 기타    | `/api/users`, `/api/blueprint`                | 계정, 조감도 이미지                  |

**하는 일**

* PostgreSQL 메타데이터 **쓰기·조회** (Repository 계층)
* Access/Refresh Token 발급, Redis에 refresh·blacklist 관리
* 임계값 변경·삭제 후 Collector `POST /internal/clear-threshold-cache` 호출
* Redis ZSET 백업 스케줄(매일), 성공 시 구간 데이터 삭제
...
* 실시간·기간 **조회 API**, WebSocket (Sensor API / Collector)
* 알람 **SMS 발송** (Collector)
* 고주파 ZIP **계산·발행** (고주파 분석)

**알아두면 좋은 것**

* 화면 표시 단위는 **`sensor_widgets`** 1행 = 위젯 1개. 채널·계산식·측정 항목은 `options` JSONB에 있다.
* **`sensor_thresholds`** 가 Collector 알람 판정의 기준이다. 위젯 CRUD만으로는 Collector 캐시가 자동으로 비워지지 않는다(임계값 API만 통보).
* 설비 계층(`projects`→`devices`)과 표시 계층(`sensor_widgets`)은 FK 없이 `site_code`·`device_id`·`channel`로 맞춘다. 상세는 관리-서버.md, erd.md.

***

### Collector (수집 서버)

센서 데이터 **입구**다. MQTT·HTTP로 측정값을 받아 Redis에 실시간 저장하고, InfluxDB에 집계 기록하며, 임계값 초과 시 SMS를 보내고 WebSocket으로 대시보드에 푸시한다.

**들어오는 경로**

| 경로   | 예                      | 처리                                                         |
| ---- | ---------------------- | ---------------------------------------------------------- |
| MQTT | `/{prefix}/+/sensdata` | 파싱 → (값 보정) → Redis → 알람 → WebSocket                       |
| MQTT | `bulk_data_aggr`       | Influx 즉시 저장 → 알람(FFT) → WebSocket                         |
| HTTP | `POST /api/lora`       | LoRa/GDMS → Influx. LoRa는 알람·WebSocket, GDMS는 알람·소켓 규칙이 다름 |

**하는 일**

* 수신 데이터 파싱·특정 센서 값 보정(`normalizeFixedValues`)
* Redis ZSET 저장, 10분 크론으로 Influx 10m 집계 → 1h·1d 롤업
* PostgreSQL에서 임계값·위젯·연락처 **읽어** 알람 판정, SMS 발송, `alarm-log` 버킷 기록
* Socket.IO `latest-data-v2`, `sensor-data-v2` 브로드캐스트
* 고주파 분석 서버가 MQTT `bulk_data_aggr`로 보낸 **집계 결과** 수신·저장

**하지 않는 일**

* 현장·장비·위젯·임계값 **CRUD** (Management)
...
* sensdata / bulk(FFT) / LoRa / GDMS마다 Redis 저장·알람 여부가 다르다. 상세는 수집-서버.md.
...
| API                           | 저장소      | 용도                        |
| ----------------------------- | -------- | ------------------------- |
| `GET /api/sensors/range`      | InfluxDB | 기간별 집계 조회 (10m / 1h / 1d) |
...
* Redis ZSET·Influx 버킷 **읽기** (Collector 저장 규칙과 동일한 파라미터 사용)
* 보정 API로 InfluxDB에 mean/max/min(또는 GDMS 문자열) **쓰기**
* KST 시각 변환, LoRa·FFT·테스트 버킷 분기 처리
...
* Redis는 약 24시간만 보관한다. 그 이전 구간은 `range`(Influx)로 조회한다.
* 상세는 센서-조회-서버.md.
...
설정·수집·조회·고주파 계산이 서버별로 나뉜다. **측정값은 Management를 거치지 않고** 고주파 분석 → MQTT → Collector → Redis/Influx → Sensor API(또는 WebSocket)로 흐른다.

```mermaid
flowchart LR
    FE["관리·대시보드"] --> MG["Management :3001"]
    FE --> SA["Sensor API :3002"]
    FE --> WS["Collector WebSocket :3003"]
...
| Management ← 고주파 분석           | 위젯 설정 **조회** (`/api/sensor-widgets/by-device-channel`)              |
| Collector → PostgreSQL        | `sensor_thresholds`, `sensor_widgets`, `projects` 등 **읽기만** (알람 판정) |
| 고주파 분석 → MQTT → Collector     | 계산 결과 `bulk_data_aggr` 발행 → Influx 즉시 저장·알람(FFT)·WebSocket          |
...
| Sensor API ↔ 고주파 분석           | HTTP 호출 없음. **Collector가 저장한 bulk/FFT 데이터**를 Influx·Redis에서 조회      |
...
| ---------------- | ------------------------------------ | ------------------------------------------------ |
| PostgreSQL       | 현장·장비·위젯·임계값 등 **메타데이터**             | Management (쓰기), Collector (읽기), 고주파 분석 (설정 조회만) |
...
센서 **측정값 본문**은 PostgreSQL에 두지 않는다. 메타는 PG, 최근 원본은 Redis, 장기·집계는 InfluxDB, 고주파 차트는 분석 서버 로컬로 나뉜다.

#### PostgreSQL

관계형 DB. 테이블·FK·JSONB 컬럼으로 **현장 구조와 화면·알람 설정**을 저장한다. 트랜잭션과 스키마가 필요한 관리 데이터에 적합하다.

**이 프로젝트에서의 사용**

* **Management** — CRUD의 기준 저장소. `management/database/pg-client.js`가 연결·스키마 초기화(`initializeTables`)를 담당하고, 도메인별 Repository가 SQL을 실행한다.
...
설비 계층과 표시 계층(`sensor_widgets`)은 FK로 직접 연결되지 않고, `site_code`·`device_id`·`channel` 문자열로 수집 데이터와 맞춘다. 상세 스키마는 erd.md 참고.
...
| 용도              | 키 패턴                                                       | 서버                     | 설명                                                                               |
| --------------- | ---------------------------------------------------------- | ---------------------- | -------------------------------------------------------------------------------- |
| 센서 실시간 캐시       | `timeseries:{sensorId}:{channel}`                          | Collector → Sensor API | ZSET. score=UTC ms, member=측정 JSON. MQTT 파싱 직후 저장, **약 24시간** 구간만 유지(저장 시·배치 정리) |
...
Redis에 없는 **24시간 이전** 구간은 Sensor API에서 InfluxDB 기간 조회로 보완한다.

#### InfluxDB 2.x

시계열(time-series) DB. InfluxDB **2.x**는 Org·Bucket·Token 인증과 **Flux** 쿼리를 사용한다. 측정값을 시간 축으로 쌓고, 구간별 평균·최대·최소 같은 **집계 보관**에 맞다.

**이 프로젝트에서의 사용**

| 구분          | 버킷·measurement                                                     | 쓰기                                                       | 읽기                              |
| ----------- | ------------------------------------------------------------------ | -------------------------------------------------------- | ------------------------------- |
| MQTT 센서 집계  | `{sensorId}_{channel}_{10m\|1h\|1d}` / `sensor_10min_aggregated` 등 | Collector (10분 배치 → 시간·일 롤업)                             | Sensor API `/api/sensors/range` |
...
* **Collector** — Redis 10분 배치 집계, bulk/LoRa/GDMS 즉시 기록, Flux 기반 1h·1d 롤업(`collector/influx/`). 버킷 없으면 생성(기본 보관 약 2년).
