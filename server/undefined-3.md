...
│   │   └── routes.py                           # 업로드·자동 처리·상태 조회 API
│   ├── calculators/                            # 센서 데이터 계산기
│   │   ├── __init__.py                         # Python 패키지 표시 파일
│   │   ├── acceleration_calculator.py          # 고유진동수와 가속도 통계 계산
│   │   ├── demo_calculator.py                  # 고정값을 반환하는 예제 계산기
│   │   ├── raw_acceleration_calculator.py      # 원시 가속도 통계 계산
│   │   └── tension_calculator.py               # 케이블 장력 계산
│   ├── core/                                   # 공통 설정과 예약 작업
│   │   ├── api/                                # 향후 API 구조를 위한 빈 폴더
...
│   │   ├── __init__.py                         # Python 패키지 표시 파일
│   │   ├── calculator_registry.py              # 계산기 이름과 실행 함수 연결
│   │   ├── file_processor.py                   # 업로드 파일 처리와 계산 설정 조회
│   │   ├── pipeline_processor.py               # 자동 다운로드·계산·발행 처리
│   │   └── status_processor.py                 # 서버와 수집 상태 응답 구성
│   ├── services/                               # 다운로드·압축·차트·MQTT 기능
│   │   ├── __init__.py                         # Python 패키지 표시 파일
│   │   ├── chart_generator.py                  # 차트 공통 설정과 생성 함수
│   │   ├── chart_plots.py                      # 차트 종류별 그리기
│   │   ├── data_processor.py                   # TXT 파일의 첫 번째 데이터 추출
│   │   ├── decompressor.py                     # ZIP 파일 검증과 압축 해제
│   │   ├── http_downloader.py                  # 고주파 ZIP 자동 다운로드
│   │   └── mqtt_publisher.py                   # 계산 결과 MQTT 발행
│   ├── utils/                                  # 공통 변환 기능
│   │   ├── __init__.py                         # Python 패키지 표시 파일
│   │   └── filename_parser.py                  # 업로드 파일명 해석과 검증
│   ├── __init__.py                             # Python 패키지 표시 파일
│   └── main.py                                 # FastAPI 서버 실행 진입점
├── data/                                       # 실행 중 입력 파일과 차트 저장
│   ├── acceleration/                           # 가속도 FFT 차트 저장
│   ├── downloads/                              # 자동 다운로드 ZIP 저장
│   ├── extracted/                              # ZIP 임시 압축 해제
│   ├── fft/                                    # 장력 FFT 차트 저장
│   ├── raw/                                    # 장력 시간 영역 차트 저장
│   ├── raw_acceleration/                       # 가속도 시간 영역 차트 저장
│   └── uploads/                                # API로 받은 원본 ZIP 저장
├── .env                                        # 실행 환경 변수 설정
...
```mermaid
flowchart TB
    C[센서 수집기 / 운영자] --> APP["FastAPI 서버 (main.py)"]
...
| `app/services/chart_generator.py`       | 차트 공통 스타일과 종류별 보조 함수                                           |
