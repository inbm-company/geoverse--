...
flowchart LR
    ...
    API -->|실시간·최신값| RR["redis-reader"] --> RD[("Redis ZSET")]
...
| 저장소      | 구조                                                                                           | 용도         |
| -------- | -------------------------------------------------------------------------------------------- | ---------- |
| Redis    | ZSET `timeseries:{sensorId}:{channelName}`, score = UTC 밀리초, member = `{ timestamp, ...필드 }` | 실시간·최신값 조회 |
...
| 파일                                         | 역할                                |
| ------------------------------------------ | --------------------------------- |
| `sensor-api/server.js`                     | Express 부트스트랩, 클라이언트 생성·주입, 종료 처리 |
| `sensor-api/api.js`                        | 라우트 정의, 수동 보정 기록 헬퍼               |
| `sensor-api/data-access/redis-reader.js`   | ZSET 조회와 구간 통계 계산                 |
...
| 문서                                                   | 내용                   |
| ---------------------------------------------------- | -------------------- |
| [수집-서버.md](%EC%88%98%EC%A7%91-%EC%84%9C%EB%B2%84.md) | 저장 구조와 버킷·키 규칙의 원본   |
| [관리-서버.md](%EA%B4%80%EB%A6%AC-%EC%84%9C%EB%B2%84.md) | 센서·현장 메타데이터와 테이블 스키마 |
