...
버킷 이름 규칙은 `influx-reader.js`에 `getBucketName`이 복사되어 있으며 수집 서버의 `bucket-utils.js`와 같은 규칙이다.
...
4. **`ensureBucket`과 버킷 이름 규칙이 수집 서버와 중복 구현되어 있다.** 한쪽 규칙을 바꾸면 다른 쪽도 함께 수정해야 한다.
5. **날씨 캐시는 프로세스 메모리에 있다.** 서버를 재시작하면 초기화되고, 인스턴스를 여러 개 띄우면 공유되지 않는다.
6. **보정 입력은 인증 절차가 없다.** 값을 직접 덮어쓰는 경로이므로 호출 범위를 네트워크 수준에서 제한해야 한다.
...
| 대상 | 상태 |
| --- | --- |
| `redis-reader.getLatestNWithRange`, `aggregateBySeconds` | 호출되지 않음 |
| `influx-reader.get10MinAggregatedData`, `format10MinAggregatedData` | 호출되지 않음 |
| `utils/timestamp.js`의 `parseToMs`, `formatToSensorTime`, `isoToMs`, `msToIso`, `roundToHour`, `roundToDay` | 이 서버에서는 호출되지 않음. 수집 서버 파일의 복사본 |
| `config.sensorApi.mongoUri`, `mongodb` 의존성 | 사용하지 않음 |
| `sensor_10min_aggregated_test` measurement | `isTest=true` 요청 전용 |
