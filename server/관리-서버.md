...
화면 표시 단위는 **`sensor_widgets`(위젯)** 이다. `/api/sensor-groups`는 4+1 구조(`sensor_groups` → `sensors`)용 병행 API로 남아 있으며, 수집 서버·알람 판정은 이 경로를 사용하지 않는다.
...
| UI 개념       | DB/API                                        | 비고                            |
| ----------- | --------------------------------------------- | ----------------------------- |
| 위젯          | `sensor_widgets` 1행, `/api/sensor-widgets`    | 운영 표시 단위                      |
| 위젯 내 센서(채널) | `options.sensorList[]` 항목                     | `deviceId`, `channel`, `name` |
| 위젯 내 측정 항목  | `sensorList[].values` 또는 `sensorList[].items` | `parseType`에 따라 필드명이 다름       |
| (병행) 센서 그룹  | `sensor_groups` 1행, `/api/sensor-groups`      | 수집 서버 미사용                     |
| (병행) 센서     | `sensors` 1행                                  | 그룹 소속 표시 항목. 수집 서버 미사용        |

수집 서버의 알람 판정은 `sensor_thresholds`, `sensor_widgets.options`, `projects.alarm_contacts`만 읽고 `devices`, `sensor_groups`, `sensors` 테이블은 조회하지 않는다.
...
운영 표시 단위. 위젯 1행이 화면의 위젯 1개에 대응하고, 채널·측정 항목·계산식은 `options` JSONB에 담긴다.
...
| `order_index`               | INTEGER      | DEFAULT 0                           |
| `options`                   | JSONB        | NOT NULL DEFAULT `{}`, 채널·센서·계산식 정의 |
| `is_active`                 | BOOLEAN      | DEFAULT TRUE                        |
...
병행 API용. UI의 "센서 그룹" 개념은 운영상 `sensor_widgets` 1행으로 대체된다.
...
병행 API용. UI의 "센서(측정 항목)" 개념은 운영상 `options.sensorList` + `values`/`items`로 대체된다.
...
`sensor_widgets.options` — 위젯 1개에 채널·센서·측정 항목을 모두 담는다.
...
`sensors.display_config` — 병행 API(`sensor_groups`/`sensors`)용. `{ unit, decimal, chartType, color, formula }` 형태로 사용하며 갱신 시 기존 값과 병합(`||`)한다. 수집 서버는 이 필드를 읽지 않는다.
...
| `GET /api/projects/:id/sensor-groups`, `GET /api/projects/site/:siteCode/sensor-groups` | 소속 센서 그룹 (병행 API)                    |
| `GET /api/projects/:id/statistics`, `GET /api/projects/site/:siteCode/statistics`       | 구성 통계 (`sensor_groups`/`sensors` 기준) |
...
운영 표시 단위. 화면의 위젯 CRUD와 수집 서버 연동 설정을 담당한다.
...
**센서 그룹 — `sensor-group-api`**

병행 API. `sensor_groups`/`sensors` 테이블 CRUD. 수집 서버·알람은 사용하지 않는다.

| 메서드·경로                                                       | 처리                                                                            |
| ------------------------------------------------------------ | ----------------------------------------------------------------------------- |
| \`GET /api/sensor-groups?projectId                           | siteCode`,` /:id`,` /project/:projectId`,` /site/:siteCode`,` /node/:nodeId\` |
...
| `getStatistics(...)`                                                                        | `project-repository.js`       | 노드·장비·그룹·센서 개수 집계 (`sensor_groups`/`sensors` 기준, 위젯 수 미포함) |
...
| `getByDeviceChannel(...)`                                                                   | `sensor-widget-repository.js` | `options.sensorList`의 `deviceId`로 위젯 조회 (수집 서버 알람·계산식)     |
...
1. **화면 표시·알람 연동은 `sensor_widgets`를 기준으로 한다.** 채널·측정 항목·계산식은 `options.sensorList`와 `values`/`items`에 저장한다. `sensor_groups`/`sensors`는 병행 API이며 수집 서버가 읽지 않는다.
2. **프로젝트 생성은 파티션 생성을 포함한다.** `sensor_groups`, `sensors`가 `project_id` 기준 LIST 파티션이므로 파티션 없이 데이터를 넣으면 실패한다.
3. **노드 이동은 하위 경로를 함께 갱신해야 한다.** `node-api`가 트랜잭션 안에서 하위 `path`를 직접 수정한다.
4. **위젯 CRUD는 수집 서버·관리 서버 캐시를 자동으로 비우지 않는다.** 임계값 변경과 달리 수집 서버 통보가 없다. 관리 서버의 `deviceChannels` 캐시는 서버 시작 시 1회 적재되며, `/by-device-channel` 조회는 TTL(`WIDGET_CHANNEL_CACHE_TTL_MS`, 기본 60초) 만료 후 DB를 다시 읽는다.
