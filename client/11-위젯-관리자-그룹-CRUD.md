...
- SWR `drawer-opened === 'widget-manager'`
- Header **위젯 관리자** (admin+)
...
| sensor type catalog | 타입·defaultOptions |
| `['cctv-cameras', siteCode]` | CCTV 카탈로그 |
...
1. API 응답 → 로컬 그룹 정규화
2. 그룹 생성·삭제·센서 배정/해제
3. 저장: create/update/reorder → API + SWR 캐시 갱신

### 1.5 그룹 종류

| widgetType | 종류 |
|------------|------|
| `sensor-widget`, `widget`, `folder` | 센서 그룹 |
| `cctv` | CCTV 그룹 (`cameraIds[]` ↔ `options.cameras[]`) |
...
1. `id`, `sensorIds`, `options` 키 규칙 변경 시 회귀 범위 넓음.
2. `dirty`와 close/저장 UX 연동 — 갱신 누락 주의.
3. sensor type catalog 수정은 **기존** 위젯 options에 영향 주면 안 됨.
...
| 문서 | 내용 |
|------|------|
| [12-위젯-관리자-DnD-인터랙션.md](./12-위젯-관리자-DnD-인터랙션.md) | DnD |
| [20-관리자-위젯-설정-이름-기본값.md](./20-관리자-위젯-설정-이름-기본값.md) | 이름 기본값 |
