...
| 파일                                  | 역할                 | 주요 함수                                                    |
| ----------------------------------- | ------------------ | -------------------------------------------------------- |
| `src/utils/SocketControl.jsx`       | 소켓 연결·정규화          | `normalizeBoardChannels`, `normalizeChannel`, `getValue` |
...
| -------------------- | ------------------------ |
| `SocketControl`      | 전역 소켓                    |
| `SensorGaugeWrapper` | `useGaugeData`로 실시간 값·색상 |

***

## 4. 구현 시 주의사항

1. 연결 상태(`Connected`)와 실제 데이터 수신은 별개 — UI에서 분리 해석.
2. 백그라운드 탭에서 연결 해제 후 복귀 시 재연결한다.
